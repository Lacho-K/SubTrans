    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (true)
            {
                ...
                if (ReadSubs())
                {
                    ...
                    List<string> SubGroups = (from Match m in Regex.Matches(Text4Trans, @".{1,2500}")
                                                  select m.Value).ToList();

                    foreach (string SubChunk in SubGroups)
                    {
                        BGText += t.translate("en", "bg", SubChunk);
                    }
        ...
        public static void TranslateSubs()
        {
            using (StreamReader reader = new StreamReader(Subs4Trans))
            {
                ...
                string[] orderedBGSubs = BGText.Split('~').Select(w => w.Trim()).Where(w => !string.IsNullOrWhiteSpace(w)).ToArray();
